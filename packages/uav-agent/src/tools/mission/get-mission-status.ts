...
export function getMissionStatusTool(capabilities: UavCapabilityClient): ToolDefinition {
	return defineTool({
		...
		async execute(_toolCallId, params, signal) {
			const status = await capabilities.getMissionStatus({ missionId: params.missionId }, signal);
			return {
				content: [
					{
						type: "text",
