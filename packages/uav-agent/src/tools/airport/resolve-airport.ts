...
export function resolveAirportTool(capabilities: UavCapabilityClient): ToolDefinition {
	return defineTool({
		...
		async execute(_toolCallId, params, signal) {
			const resolved = await capabilities.resolveAirport({ airportId: params.airportId }, signal);
			return {
				content: [
					{
						type: "text",
